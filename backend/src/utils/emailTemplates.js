...
const absoluteUrl = (value = '') => {
  ...
  const apiBase = (process.env.API_URL || process.env.NEXT_PUBLIC_API_URL || '').replace(/\/api\/?$/, '').replace(/\/$/, '');
  const finalBase = base || apiBase;
  return finalBase ? `${finalBase}${value.startsWith('/') ? value : `/${value}`}` : value;
};
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e5e7eb;box-shadow:0 12px 30px rgba(15,23,42,0.08);">
          <tr>
            <td style="background:${brand.primaryColor};padding:28px;text-align:center;color:#ffffff;">
...
const detailsTable = (rows = []) => `
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;background:#f9fafb;border:1px solid #e5e7eb;border-radius:14px;margin:22px 0;overflow:hidden;">
    ${rows.map(([label, value]) => `
      <tr>
        <td style="padding:12px 14px;color:#6b7280;font-size:13px;border-bottom:1px solid #e5e7eb;">${escapeHtml(label)}</td>
        <td style="padding:12px 14px;color:#111827;font-size:13px;font-weight:700;text-align:right;border-bottom:1px solid #e5e7eb;">${escapeHtml(value)}</td>
      </tr>`).join('')}
  </table>`;

const buildSignupEmail = async ({ student }) => {
  const brand = await getBranding();
  const portalUrl = process.env.FRONTEND_URL || process.env.BASE_URL || '';
  const content = `
    <p style="margin:0 0 16px;color:#374151;font-size:15px;line-height:1.7;">Hello ${escapeHtml(student.full_name || student.fullName || 'Student')}, your student account has been created successfully.</p>
    ${detailsTable([
      ['Index Number', student.student_id || student.indexNumber || ''],
      ['Email', student.email || ''],
      ['Programme', student.programme || ''],
      ['Academic Year', student.academic_year || student.academicYear || '']
    ])}
  ...
  return {
    ...
    text: `Hello ${student.full_name || student.fullName || 'Student'}, your account has been created successfully on ${brand.appName}.`,
    html: emailShell({ brand, title: 'Welcome to your student portal', preheader: 'Your account has been created successfully.', content, ctaText: portalUrl ? 'Open Student Portal' : '', ctaUrl: portalUrl })
  };
};
...
const buildPaymentReceiptEmail = async ({ student, payment, receipt, receiptUrl }) => {
  ...
  const content = `
    <p style="margin:0 0 16px;color:#374151;font-size:15px;line-height:1.7;">Hello ${escapeHtml(student.full_name || 'Student')}, your payment has been confirmed and your receipt is ready.</p>
    ${detailsTable([
      ['Receipt Number', receipt?.receipt_number || ''],
      ['Due', payment?.due_name || 'Payment'],
      ['Amount Paid', `GHS ${amount}`],
      ['Payment Method', String(payment?.payment_method || '').replace(/_/g, ' ').toUpperCase()],
      ['Date', new Date(payment?.created_at || Date.now()).toLocaleDateString('en-GH')]
    ])}
    <p style="margin:0;color:#6b7280;font-size:13px;line-height:1.7;">A PDF copy may be attached. You can also download it from your student dashboard.</p>`;
  return {
    subject: `Payment Receipt ${receipt?.receipt_number || ''} - ${brand.appName}`,
    text: `Hello ${student.full_name || 'Student'}, your payment of GHS ${amount} has been confirmed. Receipt: ${receipt?.receipt_number || ''}. ${receiptUrl || ''}`,
    html: emailShell({ brand, title: 'Payment Confirmed', preheader: 'Your payment receipt is ready.', content, ctaText: receiptUrl ? 'View Receipt' : (portalUrl ? 'Open Dashboard' : ''), ctaUrl: receiptUrl || portalUrl })
  };
};
...
const buildClearanceEmail = async ({ student, clearanceStatus, attachmentNote = '' }) => {
  ...
  const portalUrl = process.env.FRONTEND_URL || process.env.BASE_URL || '';
  const content = `
    <p style="margin:0 0 16px;color:#374151;font-size:15px;line-height:1.7;">Hello ${escapeHtml(student.full_name || 'Student')}, your clearance document has been generated.</p>
    ${detailsTable([
      ['Index Number', student.student_id || ''],
      ['Programme', student.programme || ''],
      ['Status', clearanceStatus || 'Generated'],
  ...
  return {
    subject: `Clearance Document - ${brand.appName}`,
    text: `Hello ${student.full_name || 'Student'}, your clearance document has been generated. Status: ${clearanceStatus || 'Generated'}.`,
    html: emailShell({ brand, title: 'Clearance Document Generated', preheader: 'Your clearance document is ready.', content, ctaText: portalUrl ? 'Open Portal' : '', ctaUrl: portalUrl })
  };
};
...
const buildExportEmail = async ({ recipientName = 'Admin', exportName = 'Export File' }) => {
  const brand = await getBranding();
  const content = `
    <p style="margin:0 0 16px;color:#374151;font-size:15px;line-height:1.7;">Hello ${escapeHtml(recipientName)}, your requested export is ready.</p>
    ${detailsTable([
      ['Export', exportName],
      ['Generated On', new Date().toLocaleString('en-GH')]
    ])}
    <p style="margin:0;color:#6b7280;font-size:13px;line-height:1.7;">The export file is attached where supported.</p>`;
  return {
    subject: `${exportName} - ${brand.appName}`,
    text: `${exportName} is ready.` ,
    html: emailShell({ brand, title: 'Export Ready', preheader: 'Your export file is ready.', content })
  };
};
